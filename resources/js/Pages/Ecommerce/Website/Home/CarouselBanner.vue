<template>
  ...
  <div class="relative w-full overflow-hidden">
    ...
    <button @click="prevSlide"
      class="absolute bottom-0 left-0 top-0 z-[1] flex items-center justify-center w-[5%] text-white hover:opacity-70 transition-opacity duration-150 ease-in-out focus:outline-none">
    ...
    <button @click="nextSlide"
      class="absolute bottom-0 right-0 top-0 z-[1] flex items-center justify-center w-[5%] text-white hover:opacity-70 transition-opacity duration-150 ease-in-out focus:outline-none">
    ...
    <div class="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
      <button v-for="(banner, index) in banners" :key="banner.id" @click="goToSlide(index)"
        :class="{ 'bg-white dark:bg-black': currentSlideIndex === index, ' bg-black dark:bg-white': currentSlideIndex !== index }"
...
const handleResize = () => {
  // Não há necessidade de atualizar currentSlideIndex explicitamente
};

onMounted(async () => {
  try {
    const response = await axios.get('/api/website/siteSlide');
