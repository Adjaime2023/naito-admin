<template>
  ...
  // Inicializar grupos como uma referência reativa
  const groups = ref(props.groups || []);
  
  // Criar menuItems dinamicamente
  const menuItems = ref([
    { label: "Home", url: "home" },
  ...
  onMounted(() => {
    console.log('Props groups:', props.groups); // Verificar se os grupos estão sendo recebidos
    console.log('Groups ref:', groups.value); // Verificar se a referência está correta
  
    // Atualizar submenu com base nos grupos de produtos
    const produtosItem = menuItems.value.find(item => item.label === "Produtos");
    if (produtosItem) {
      produtosItem.submenu = groups.value.map(group => ({
        label: group.name,
        url: route('websiteProducts', { group_id: group.id })
      }));
      console.log('Updated Produtos submenu:', produtosItem.submenu); // Verificar se o submenu foi atualizado corretamente
    }
  });
