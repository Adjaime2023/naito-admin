<template>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
        <h2 class="text-lg font-bold mb-4">Cadastrar Novo Subgrupo de Produto</h2>
        <form @submit.prevent="submitNew">
            <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
                <div class="col-span-1 sm:col-span-1 md:col-span-1">
                    <InputLabel for="main_group_id">Grupo:</InputLabel>
                    <select
            ...
            <button @click="closeModalEspecificSubgroup"
...
const cancelForm = () => {
    form.reset('group_id', 'name', 'description', 'discontinued', 'deleted');
};

const closeModalSubgroup = () => {
...
const submitNew = async () => {
    ...
    try {
        await form.post(route('subgroup.store'));
        form.processing = false;
        closeModalSubgroup(); // Fecha o modal após o sucesso
    } catch (error) {
