...
const submitNewCategory = async () => {
  ...
  try {
    ...
    closeModalCategory(); // Fecha o modal após o sucesso
  } catch (error) {
...
<template>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
      <h2 class="text-lg font-bold mb-4">Cadastrar Nova Categoria</h2>
      <form @submit.prevent="submitNewCategory">
        <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
          ...
          <div class="col-span-1 sm:col-span-3 md:col-span-3">
            ...
            <textarea class="form-control mt-2 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" v-model="form.description" id="description"></textarea>
        ...
        <div class="flex items-center justify-center w-full mt-4">
          <PrimaryButton @click.prevent="cancelForm" class="w-1/3 sm:w-1/4 md:w-1/5 lg:w-1/6">Limpar</PrimaryButton>
          <ButtonSubmit class="w-1/3 sm:w-1/4 md:w-1/5 lg:w-1/6" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Cadastrar</ButtonSubmit>
          <ButtonLink href="" class="w-1/3 sm:w-1/4 md:w-1/5
          lg:w-1/6">Listar</ButtonLink>
        </div>
        <button @click="closeModalCategory" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded mt-4">Fechar Modal</button>
      </form>
    </div>
  </template>
