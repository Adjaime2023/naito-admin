<template>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
        <h2 class="text-lg font-bold mb-4">Cadastrar Novo Grupo de Produto</h2>
        <form @submit.prevent="submitNew">
            <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
                <div class="col-span-1 sm:col-span-1 md:col-span-1">
                    <InputLabel for="main_subcategory_id">Subcategoria:</InputLabel>
                    <select
                        ...
                        <option v-for="subcategory in props.subcategories" :key="subcategory.id" :value="subcategory.id">
                            {{ subcategory.name }}
                        </option>
                    </select>
...
const cancelForm = () => {
    form.reset('subcategory_id', 'name', 'description', 'discontinued', 'deleted');
};
...
const submitNew = async () => {
    ...
    try {
        ...
        closeModalGroup(); // Fecha o modal após o sucesso
    } catch (error) {
