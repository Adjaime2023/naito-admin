<template>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
      <h2 class="text-lg font-bold mb-4">Cadastrar Nova Cor</h2>
      <form @submit.prevent="submitNew">
        ...
        <button @click="closeModalColor" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded mt-4">Fechar Modal</button>
      </form>
    </div>
  </template>
  ...
  const submitNew = async () => {
    ...
    form.post(route('color.store'))
      .then(() => {
        ...
        closeModalColor(); // Fecha o modal após o sucesso
      })
      .catch(error => {
