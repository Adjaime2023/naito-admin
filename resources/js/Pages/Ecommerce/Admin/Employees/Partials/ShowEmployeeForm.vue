...
const fetchEmployees = async () => {
    try {
        let url = `/api/employees?page=${currentPage.value}&per_page=${itemsPerPage.value}&sortField=${sortField.value}&sortDirection=${sortDirection.value}`;
        ...
        console.log('Fetching employees with URL:', url); // Log the URL

        const response = await axios.get(url);
        console.log('Response data:', response.data); // Log the response data

        employees.value = response.data.data; // Assuming your API returns data inside 'data'
...
const gotoPage = (pageNumber) => {
    ...
    fetchEmployees(); // Chame a função para buscar os parceiros da página selecionada
};

const nextPage = () => {
    if (currentPage.value < totalPages.value) {
        currentPage.value++;
        fetchEmployees(); // Chame a função para buscar os parceiros da próxima página
    }
};
...
const previousPage = () => {
    if (currentPage.value > 1) {
        currentPage.value--;
        fetchEmployees(); // Chame a função para buscar os parceiros da página anterior
    }
};



const toggleDiscontinued = async (employee) => {
    try {
        const updateDiscontinued = !employee.discontinued;
        const response = await axios.patch(`/api/employees/${employee.id}/discontinued`, {
            discontinued: updateDiscontinued
        });

        if (response.status === 200) {
            employee.discontinued = updateDiscontinued;
        } else {
...
// Função para alternar o estado de deletado do Funcionário
const toggleDeleted = async (employee) => {
    try {
        const updatedDeleted = !employee.deleted_at; // Inverte o estado atual

        // Requisição PATCH para atualizar o estado de deletado no servidor
        const response = await axios.patch(`/api/employees/${employee.id}/deleted`, {
            deleted: updatedDeleted
        });

        if (response.status === 200) {
            employee.deleted_at = updatedDeleted ? new Date().toISOString() : null; // Atualiza localmente
        } else {
            console.error('Erro ao atualizar Funcionário');
        }
    } catch (error) {
                    ...
                    <div
                        ...
                        <div class="flex items-center">
                            ...
                            <input
                                ...
                                type="checkbox" v-model="discontinued" id="discontinued" @change="fetchEmployees" />
                        ...
                        <div class="flex items-center">
                            ...
                            <input
                                class="border-x-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                type="checkbox" v-model="deleted" id="deleted" @change="fetchEmployees" />
                            <span class="ml-1 text-red-500"
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr
                                ...
                                <th class="px-1 py-1 text-xs">
                                    <div class="flex items-center">
                                        <a @click="sortBy('office_name')"
                                            :class="{ 'text-xs': true, 'cursor-pointer': true }">
                                            Função
                                            <span v-if="sortField === 'office_name'">
                                                <span v-if="sortDirection === 'asc'">&#x25B2;</span>
                                                <span v-else-if="sortDirection === 'desc'">&#x25BC;</span>
                                            </span>
                                        </a>
                                ...
                                <th class="px-1 py-1 text-xs">
                                    <div class="flex items-center">
                                        <a @click="sortBy('cpf')" :class="{ 'text-xs': true, 'cursor-pointer': true }">
                                            CPF
                                            <span v-if="sortField === 'cpf'">
                                                <span v-if="sortDirection === 'asc'">&#x25B2;</span>
                                                <span v-else-if="sortDirection === 'desc'">&#x25BC;</span>
                                            </span>
                                        </a>
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-900">
                            <tr v-for="employee in employees" :key="employee.id"
                                ...
                                <td class="px-1 py-1 text-xs">{{ employee.name || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.contact_name || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.offices || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.cpf || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.email || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.whatsapp || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.phone || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.city || 'N/A' }}</td>
                                <td class="px-1 py-1 text-xs">{{ employee.cep || 'N/A' }}</td>

                                <td class="px-4 py-2 text-sm">
                                    <td class="px-1 py-1 text-sm flex items-center justify-between">
                                    <a :href="`/employee/${employee.id}/edit`"
                                        :class="{ 'opacity-50 cursor-not-allowed': employee.deleted_at }"
                                    ...
                                    <button @click="toggleDiscontinued(employee)"
                                      :class="{ 'opacity-50 cursor-not-allowed': employee.deleted_at }"
                                        :disabled="employee.deleted_at"
                                        class="flex items-center justify-between px-1 py-1 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-blue-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Descontinuar">
                                        <i v-if="!employee.discontinued" class="fa-solid fa-file-import"
                                    ...
                                    <button @click="toggleDeleted(employee)"
                                        class="flex items-center justify-between px-1 py-1 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-blue-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Deleted">
                                        <i v-if="!employee.deleted_at" class="fa-solid fa-trash"
