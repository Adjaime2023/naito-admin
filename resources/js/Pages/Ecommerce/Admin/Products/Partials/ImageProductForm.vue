    ...
    <div class="mx-2 px-4 pt-4 pb-6 bg-gray-200 dark:bg-gray-800 rounded-lg">
        <form @submit.prevent="submitForm" method="post" enctype="multipart/form-data">
            <div class="grid justify-items-stretch items-center col-span-1 sm:col-span-1 md:col-span-1">
                ...
                <InputLabel for="image">Imagem Principal </InputLabel>

                <div class="flex justify-center mt-2">
                    <img :src="imagePreview" :alt="form.name" class="rounded max-sm:h-40 w-full object-cover">
                </div>
...
const form = useForm({
    _method: 'PUT',
    image: props.product.image, // Inicializa com a imagem existente do produto
});

// Variável reativa para a pré-visualização da imagem
const imagePreview = ref(props.product.image ? `/storage/${props.product.image}` : '/storage/images/default.png');
...
function submitForm() {
    ...
    form.post(route('product.updateImage', { id: props.product.id }), {
        onSuccess: () => {
